...
double pot_limit_2d(double y) {
    return 1 / y + gsl_sf_bessel_J0(y) * (std::log(0.5 * y) + M_EULER) +
...
std::vector<double> wf_2d_static_py(
    ...
    auto [f_vec, t_vec] = wf_s<true, 3, 2>(E, mu_e, sys);
    std::vector<double> r(3 * f_vec.size());
...
#pragma omp parallel for
    for (uint64_t i = 0; i < N_mu; i++) {
        output[i] = wf_E<3, 2>(mu_vec[i], sys);
    }
