...
double mb_2d_iod(double n, double lambda_s, const system_data& sys) {
    ...
    double v{2 * std::pow(2 * M_PI, 2) /
             (n * (sys.m_pe * sys.m_ph / sys.m_sigma)) *
             std::exp(b_ex / (4 * M_PI))};
...
double analytic_2d_n_sc(double mu_t, double chi_ex, const system_data& sys) {
    ...
    gsl_integration_qagiu(
        &integrand1, 0, 0, global_eps, local_ws_size, ws, result, error);
    gsl_integration_qagiu(
        &integrand2, -1 / chi_ex, 0, global_eps, local_ws_size, ws, result + 1,
        error + 1);
...
double ideal_2d_n_dmu(double mu_i, double m_pi) {
    return 0.25 * M_1_PI * M_1_PI /
           (m_pi * (1 + std::exp(-0.25 * M_1_PI * mu_i)));
}

double analytic_2d_n_ex_dmu(
    double mu_t, double chi_ex, const system_data& sys) {
    return sys.m_sigma * 0.25 * M_1_PI / (std::exp(chi_ex - mu_t) - 1);
}

double analytic_2d_n_sc_dmu(
    double mu_t, double chi_ex, const system_data& sys) {
    return derivative_b3(analytic_2d_n_sc, mu_t, 1e-6 * mu_t, chi_ex, sys);
}

double analytic_2d_n_ex_dchi(
    double mu_t, double chi_ex, const system_data& sys) {
    return sys.m_sigma * 0.25 * M_1_PI / (1 - std::exp(chi_ex - mu_t));
}

double analytic_2d_n_sc_dchi_f(
    double chi_ex, double mu_t, const system_data& sys) {
    return analytic_2d_n_sc(mu_t, chi_ex, sys);
}

double analytic_2d_n_sc_dchi(
    double mu_t, double chi_ex, const system_data& sys) {
    return derivative_b3(
        analytic_2d_n_sc_dchi_f, chi_ex, 1e-6 * chi_ex, mu_t, sys);
}
...
double ideal_2d_mu_h(double mu_e, const system_data& sys) {
    return 4 * M_PI *
           std::log(
               std::exp(
                   sys.m_ph / sys.m_pe *
                   std::log(1 + std::exp(0.25 * mu_e * M_1_PI))) -
               1);
}
...
double ideal_2d_mu_h_dmu(double mu_e, const system_data& sys) {
    return sys.m_ph / sys.m_pe /
           ((1 + std::exp(-0.25 * M_1_PI * mu_e)) *
            (1 - std::exp(
                     -sys.m_ph / sys.m_pe *
                     std::log(1 + std::exp(0.25 * M_1_PI * mu_e)))));
}
...
double ideal_2d_mu_v_df(double mu_e, void* params) {
    ...
    return 1 +
           m_ratio / ((1 + std::exp(-0.25 * M_1_PI * mu_e)) *
                      (1 - std::exp(
                               -m_ratio *
                               std::log(1 + std::exp(0.25 * M_1_PI * mu_e)))));
}
...
double ideal_2d_ls_mu(double mu_e, double mu_h, const system_data& sys) {
    return (1 / ((1 + std::exp(-0.25 * M_1_PI * mu_e)) * sys.m_pe) +
            1 / ((1 + std::exp(-0.25 * M_1_PI * mu_h)) * sys.m_ph)) /
           (2 * M_PI * M_PI * sys.eps_r /
            (sys.c_aEM * std::sqrt(2 * sys.m_pT)));
}
...
std::vector<double> analytic_2d_mu(double n, const system_data& sys) {
    ...
    double x_init[n_eq] = {init_u, init_v};

    // printf("first: %.3f, %.10f, %.10f, %.10f, chi_ex: %.2f\n", n, x_init[0],
    // x_init[1], ideal_2d_mu_h(init_mu, sys), chi_ex);
    ...
    for (int status = GSL_CONTINUE, iter = 0;
        ...
        /*
        printf("x: ");
        for(size_t i = 0; i < n_eq; i++) { printf("%f, ", gsl_vector_get(s->x,
        i)); }

        printf("f: ");
        for(size_t i = 0; i < n_eq; i++) { printf("%e, ", gsl_vector_get(s->f,
        i)); } printf("\n");
        */
    }
    ...
    r[0] = ideal_2d_mu_v(
        -std::log(1 + std::exp(gsl_vector_get(s->x, 0))) +
            wf_E<2, 2>(r[1], sys),
        sys);
...
double analytic_2d_mu_f_py(
    double mu_e, double /*ls*/, double n, const system_data& sys) {
    ...
    if (chi_ex > mu_t) {
        ...
        // return std::pow(- n + n_id + n_ex + n_sc, 2) + std::pow(- new_ls +
        // ls, 2);
...
double analytic_2d_mu_f_optim(
    ...
    double ls{s->ls_max / (std::exp(-v) + 1)};
    double mu_e{ideal_2d_mu_v(wf_E<2, 2>(ls, s->sys) - std::exp(u), s->sys)};
...
std::vector<double> analytic_2d_mu_optim(double n, const system_data& sys) {
    ...
    double x_init[n_eq] = {0, 0};

    // printf("first: %.3e, %.10f, %.10f, %.10f, chi_ex: %.2f\n", n, x_init[0],
    // x_init[1], low_mu_h, chi_ex);
