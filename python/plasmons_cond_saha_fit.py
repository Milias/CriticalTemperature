...
data = None  #loadtxt('../data2.txt', delimiter=',').reshape((1, n_vec.size + 2))
p_data = None #loadtxt('../data_points.txt', delimiter=' ').reshape((8, ))
...
for c, (i, T) in zip(colors, enumerate(T_vec)):
    ...
    real_x = (sys.c_e_charge / L * p_Na_vec * saha_p_q_yield_vec).reshape(
        p_data.size - 2, 1)
    real_y = exp_points[:, 1]
    ...
    imag_x0 = sys.c_e_charge * p_Na_vec / L * saha_p_q_yield_vec
    imag_x1 = p_Na_vec / L * freq * 2 * pi * saha_p_q_yield_exc_vec
    ...
    err_mob_R, err_mob_I, err_pol = real_fit.bse, *imag_fit.bse
