...
eps_sol = 1.9955
eps_r = root_scalar(
    lambda eps_r: exciton_be_cou(system_data(m_e, m_h, eps_r, T)) - be_exc,
    bracket=[1.0, 10.0],
    method='brentq',
).root
...
print('[Keldysh] sol: %.3f, medium: %.3f' % (eps_sol, eps))
sys = system_data(m_e, m_h, eps_sol, T)

be_exc_ke = exciton_be_ke(size_d, eps, sys)
print('E_B(Cou): %.0f meV, E_B(Keldysh): %.0f meV' %
...
ax[0].axvline(x=wf_ke_vec[N_fit_cou, 2], color='r', linestyle='--')
