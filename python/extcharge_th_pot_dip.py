...
def dip_k_pot(u, x, sys, sys2):
    pot = 2 * exp(-u * (sys.ext_dist_l + 0.5 * sys.size_d) /
                  x) / (sys.eps_mat + sys.eps_r +
                        (sys.eps_r - sys.eps_mat) * exp(-u * sys.size_d / x))

    pot -= 2 * exp(-u * (sys2.ext_dist_l + 0.5 * sys2.size_d) / x) / (
        sys2.eps_mat + sys2.eps_r +
        (sys2.eps_r - sys2.eps_mat) * exp(-u * sys2.size_d / x))
...
sys_mat = system_data(m_e, m_h, eps_sol, T, size_d, eps_mat, ext_dist_l1)
sys_mat2 = system_data(m_e, m_h, eps_sol, T, size_d, eps_mat, ext_dist_l2)
...
def calc_data(rho_cm, a0, th, sys, sys2, n_a0, ke_vec):
    ...
    return ke_vec[n_a0] + integ_pot(ext_k_pot, rho_e, sys) - integ_pot(
        ext_k_pot, rho_h, sys)
    #- integ_pot(ext_k_pot, rho_e, sys2) + integ_pot(ext_k_pot, rho_h, sys2)


def dip_calc_data(rho_cm, a0, th, sys, sys2, n_a0, ke_vec):
    rho_e = sqrt(rho_cm**2 + a0**2 / (1 + sys.m_eh)**2 +
                 2 * rho_cm * a0 * cos(th) / (1 + sys.m_eh))
    rho_h = sqrt(rho_cm**2 + a0**2 / (1 + 1 / sys.m_eh)**2 -
                 2 * rho_cm * a0 * cos(th) / (1 + 1 / sys.m_eh))

    return ke_vec[n_a0] + integ_pot(dip_k_pot, rho_e, sys, sys2) - integ_pot(
        dip_k_pot, rho_h, sys, sys2)


rho_cm_vec = array([0.5, 1, 4]) * sys_mat.exc_bohr_radius_mat()
...
if file_id == '':
    ...
    save_data(
        'extra/extcharge/pot_th_dip_%s' % file_id,
        [data.flatten()],
    )
else:
    data = load_data(
