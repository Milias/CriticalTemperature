...
eig_arr = array(topo_t_cou_eig(Q_val, N_k, sys)).reshape(N_k + 1, N_k)
eig_val = eig_arr[0]
eig_vec = eig_arr[1:]
...
disp_int_plot_vec = array([topo_disp_t_int(0, k, sys) for k in k_plot_vec])

k_vec = linspace(0, Q_val, N_k)
disp_int_vec = array([topo_disp_t_int(0, k, sys) for k in k_vec])
